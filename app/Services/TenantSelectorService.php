...
class TenantSelectorService
{
     ...
     *
     * TenantUser 모델을 통해 Organization/Store/Brand별로 그룹화하여 반환
     *
    ...
    {
        // TenantUser 레코드에서 각 ScopeType별 테넌트 ID 목록 조회
        $orgIds = \DB::table('tenant_users')
        ...
        $storeIds = \DB::table('tenant_users')
            ->where('user_id', $user->id)
        ...
        $brandIds = \DB::table('tenant_users')
            ->where('user_id', $user->id)
